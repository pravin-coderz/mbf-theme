...
if(isset($_REQUEST['export']) && $_REQUEST['export'] == 'enquiry_list'):
    ...
    // Read from Database.
    $query = "SELECT * FROM wp_newsletter ORDER BY `posted_date` desc";
...
class Link_List1_Table extends WP_List_Table
{
    ...
    {
        if ( $which == "top" ) {          
            echo "<h1>Newsletter Subscriber List</h1>";
    ...
    { ?>
        <br /><br /><span style="float:right; margin-bottom: 29px; margin-top: 9px;"><a href="?page=<?php echo $_REQUEST['page']?>&export=enquiry_list" id="export-enquiry_list" class="button" >Export</a></span>
        <?php
        $records = $this->items;
